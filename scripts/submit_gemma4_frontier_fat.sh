...
for numeric_name in \
  TASK_COUNT TASK_OFFSET NODES TASK_CONCURRENCY MAX_STEPS N_ACT N_VER MAX_MODEL_LEN MAX_NUM_BATCHED_TOKENS \
  RECOVERY_RWR_TOP_K RWR_RESULT_PREVIEW_LIMIT RWR_NON_SEED_PREVIEW_LIMIT RWR_NON_SEED_ID_PREVIEW_LIMIT \
  TOOL_REFERENCE_GENE_LIMIT MEMBERSHIP_EDIT_TOP_K MEMBERSHIP_EDIT_MAX_CUMULATIVE_ADDITIONS \
  MEMBERSHIP_EDIT_MAX_COMBINATION_ADDITIONS MEMBERSHIP_EDIT_MAX_COMBINATION_BRANCHES MEMBERSHIP_EDIT_MAX_DROP_PAIRS; do
...
case "${MEMBERSHIP_EDIT_BRANCHES}" in
  off|hybrid) ;;
  *) echo "MEMBERSHIP_EDIT_BRANCHES must be one of: off, hybrid" >&2; exit 2 ;;
esac
...
RUN_ENV_PATH="${RUN_ROOT}/run.env"
{
  for name in \
    ...
    GENERATOR_TIMEOUT_SECONDS RECOVERY_RWR_TOP_K RWR_RESULT_PREVIEW_LIMIT RWR_NON_SEED_PREVIEW_LIMIT \
    RWR_NON_SEED_ID_PREVIEW_LIMIT TOOL_REFERENCE_GENE_LIMIT MEMBERSHIP_EDIT_BRANCHES MEMBERSHIP_EDIT_TOP_K \
    MEMBERSHIP_EDIT_MAX_CUMULATIVE_ADDITIONS MEMBERSHIP_EDIT_MAX_COMBINATION_ADDITIONS \
    MEMBERSHIP_EDIT_MAX_COMBINATION_BRANCHES MEMBERSHIP_EDIT_MAX_DROP_PAIRS; do
    quote_env "${name}"
  done
...
run_lane() {
  ...
  (
    ...
    export RECOVERY_RWR_TOP_K RWR_RESULT_PREVIEW_LIMIT RWR_NON_SEED_PREVIEW_LIMIT RWR_NON_SEED_ID_PREVIEW_LIMIT
    export TOOL_REFERENCE_GENE_LIMIT MEMBERSHIP_EDIT_BRANCHES MEMBERSHIP_EDIT_TOP_K
    export MEMBERSHIP_EDIT_MAX_CUMULATIVE_ADDITIONS MEMBERSHIP_EDIT_MAX_COMBINATION_ADDITIONS
    export MEMBERSHIP_EDIT_MAX_COMBINATION_BRANCHES MEMBERSHIP_EDIT_MAX_DROP_PAIRS
    ...
    echo "ENV_BEGIN"
    env | sort | grep -E '^(MODEL|SIF|MAX_|N_ACT|N_VER|TASK_|GENERATOR_|VLLM_|STAGE_|OUT_DIR|RUN_SMOKE|PREFETCH|RWR_HPC|RECOVERY_RWR|RWR_RESULT|RWR_NON_SEED|TOOL_REFERENCE|MEMBERSHIP_|REPO_ROOT|DATA_ROOT|SCRATCH|LOCAL_NVME_ROOT|SLURM_JOB_NODELIST|SLURM_NNODES)=' || true
    echo "ENV_END"
...
cat > "${RUN_ROOT}/submit_config.json" <<EOF_JSON
{
  ...
  "recovery_rwr_top_k": "${RECOVERY_RWR_TOP_K}",
  "rwr_result_preview_limit": "${RWR_RESULT_PREVIEW_LIMIT}",
  "rwr_non_seed_preview_limit": "${RWR_NON_SEED_PREVIEW_LIMIT}",
  "rwr_non_seed_id_preview_limit": "${RWR_NON_SEED_ID_PREVIEW_LIMIT}",
  "tool_reference_gene_limit": "${TOOL_REFERENCE_GENE_LIMIT}",
  "membership_edit_branches": "${MEMBERSHIP_EDIT_BRANCHES}",
  "membership_edit_top_k": "${MEMBERSHIP_EDIT_TOP_K}",
  "membership_edit_max_cumulative_additions": "${MEMBERSHIP_EDIT_MAX_CUMULATIVE_ADDITIONS}",
  "membership_edit_max_combination_additions": "${MEMBERSHIP_EDIT_MAX_COMBINATION_ADDITIONS}",
  "membership_edit_max_combination_branches": "${MEMBERSHIP_EDIT_MAX_COMBINATION_BRANCHES}",
  "membership_edit_max_drop_pairs": "${MEMBERSHIP_EDIT_MAX_DROP_PAIRS}"
}
EOF_JSON
