...
def main():
    ...
    # -----------------------------------------
    SYSTEM_PROMPT = """A conversation between User and Assistant. The user asks a question, and the Assistant solves it, answering "yes" or "no". The assistant first thinks about the reasoning process in the mind and then provides the user with the answer. The reasoning process and answer are enclosed within <think> </think> and <answer> </answer> tags, respectively."""
    ...
    def generate_rollout_data(model, ref_model, tokenizer, batch_samples, num_generations, max_completion_length):
        ...
        with torch.no_grad():
            ...
            ref_model_device = ref_model.to(input_ids.device)
            ref_log_probs = compute_log_probs(ref_model_device, input_ids, attention_mask, logits_to_keep)
            ref_model.to("cpu")
    ...
    def train_with_grpo(
        ...
        gbs = training_config["batch_size"] * accelerator.num_processes
        if accelerator.is_main_process:
            print(f"Per-Device Micro-Batch Size: {training_config["batch_size"]} | Global Batch size: {gbs}")
        ...
        for iteration in range(num_iterations):
            ...
            with torch.no_grad():
                # unwrap model, get state dict, make ref model, use state dict for peft init on cpu
                unwrapped_model = accelerator.unwrap_model(model)
                state_dict = {k: v.cpu() for k, v in unwrapped_model.state_dict().items()} 
            ...
            # send items to accelerator
            model, optimizer, train_data = accelerator.prepare(model, optimizer, train_data)
            model.train()
            
            local_step = 0
            for batch_samples in train_data:
                if local_step >= num_steps:
                    break
                local_step += 1
                
                global_step = local_step * accelerator.num_processes
                ...
                for grpo_iter in range(mu):
                    ...
                    if accelerator.is_main_process:
                        print(
                            ...
                            f"Local step {step+1}/{num_steps}, Global step {global_step}"
    ...
    if accelerator.is_main_process:
        ...
        save_path = os.path.join(MODEL_DIR + "GRPO-EdgePred-Vanilla")
