...
To make the warm-start SFT stage concrete, we define a core tool vocabulary derived from the existing \textbf{\textsc{Mentor-IA}} infrastructure \cite{mentor-ia} in Table \ref{tbl:tools}, with each tool specified by its purpose, input schema, output schema, an-mentor-ia}d training stage. Early SFT uses only this core tool set to teach schema-valid biological tool use before larger graph-exploration operators are introduced during DPO and online RL.
...
\subsubsection{Plain-Language Map of the Trajectory Generation Artifacts}
The trajectory generator produces several files because each file answers a different debugging or training question. In simple terms, a trajectory run works as follows.

\begin{itemize}
    \item \textbf{Task row.} A task row is one biological problem sampled from the CORUM corpus. It stores the visible seed genes, the hidden target complex used for scoring, the task type, and the evidence mode. The model sees only the visible inputs, not the hidden target.
    \item \textbf{Evidence mode.} The evidence mode controls what the model is allowed to see. \texttt{minimal} shows only seed genes, \texttt{graph} shows seed genes and a graph query specification, \texttt{contextual} adds free-text context, and \texttt{full} adds graph, context, and structured annotations.
    \item \textbf{Initial interpretation and state.} The run begins with an empty human-readable interpretation and a structured state initialized from the visible task inputs. The structured state stores the current predicted group, relationship status, evidence log, mechanistic labels, tool counters, and remaining step budget.
    \item \textbf{Actor.} The actor writes the next reasoning step and may choose one tool call. The actor should use canonical gene identifiers. For graph tools, it should omit the \texttt{layers} or \texttt{layer} argument when it wants all layers; it should not write \texttt{all}, an empty list, or \texttt{null}. The runtime also normalizes these common all-layer spellings to the omitted form before validation and execution.
    \item \textbf{Runtime tool call.} If the actor asks for a tool, the runtime validates the tool name, arguments, genes, requested layers, budget, and duplicate-call status. Valid graph calls are executed against the deterministic multiplex runtime. Invalid calls are logged as invalid observations and penalized.
    \item \textbf{Observation.} An observation is the structured result of a tool call. For graph tools it can contain neighbors, paths, ranked random-walk results, or induced-subgraph edges with provenance. If no tool is called, the observation is empty by design.
    \item \textbf{Verifier.} The verifier reads the actor step and observation, then updates the interpretation and structured state. It decides whether to continue, revise, or stop. The verifier owns structured fields such as relationship status, predicted groups, mechanistic labels, and remaining uncertainty.
    \item \textbf{Candidate branch.} One candidate branch is one possible next step: actor reasoning, optional tool action, optional observation, verifier update, and local score. A model-backed run samples several actor candidates and several verifier candidates for the same shared prefix.
    \item \textbf{Branch pool.} A branch pool is the full set of candidate branches for one trajectory step. \texttt{branch\_pools.jsonl} is the best file for debugging why the generator selected one continuation over another, because it contains both chosen and rejected candidates.
    \item \textbf{Local score.} Each branch receives a deterministic score from schema validity, complex-membership improvement, mechanistic-label improvement, and efficiency. This score is used to rank branches, mine DPO pairs, and select the continuation branch.
    \item \textbf{Selected trajectory turn.} The highest-scoring branch becomes the next logged turn. \texttt{trajectory\_turns.jsonl} contains only this selected path, so it is easier to read than the full branch pools but does not show all rejected alternatives.
    \item \textbf{Finding record.} Each selected turn is converted into a short finding. \texttt{finding\_records.jsonl} is useful for checking whether the trajectory is producing concise, evidence-grounded intermediate statements.
    \item \textbf{Final summary.} At the end of each trajectory, the selected turns and findings are rendered into one final mechanistic summary. \texttt{final\_summaries.jsonl} is the easiest file for quick human inspection of the final answer and terminal structured state.
    \item \textbf{Preference pairs.} Branches that share the same prefix are compared to make DPO pairs. \texttt{preference\_pairs\_raw.jsonl} stores the mined pairs before balancing and filtering. \texttt{preference\_pairs.jsonl} stores the final training pairs.
    \item \textbf{Run metadata.} \texttt{manifest.json} records the run configuration, model endpoint, task selection, and artifact counts. \texttt{progress.json} records completion state and high-level metrics. \texttt{served\_models.json} records the vLLM model id used by the run. \texttt{vllm\_server.log} records server startup, warnings, and request throughput.
\end{itemize}

The most useful manual inspection order is usually: first read \texttt{progress.json}, then \texttt{manifest.json}, then \texttt{final\_summaries.jsonl}, then \texttt{preference\_pairs.jsonl}. If something looks wrong, inspect the matching step in \texttt{trajectory\_turns.jsonl} and finally the full candidate set in \texttt{branch\_pools.jsonl}.
